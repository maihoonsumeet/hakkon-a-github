...
import type { Club, User, Post, Funding } from './types';
...
try {
    ...
    if (savedUsers) {
        ...
        // FIX: Ensure default mock users always have their passwords active.
        // This prevents login errors if localStorage has stale data from a previous version without passwords.
        if (users['fan@example.com']) users['fan@example.com'].password = initialMockUsers['fan@example.com'].password;
        if (users['creator@example.com']) users['creator@example.com'].password = initialMockUsers['creator@example.com'].password;
    }
...
export const database = {
    ...
    findUserByEmail(email: string): User | undefined {
        ...
        const foundKey = Object.keys(users).find(key => key.toLowerCase() === lowerEmail);
        return foundKey ? users[foundKey] : undefined;
    },
    ...
    donateToClub(clubId: number, amount: number) {
        clubs = clubs.map(club => {
            if (club.id === clubId) {
                const newCurrent = club.funding.current + amount;
                return { ...club, funding: { ...club.funding, current: newCurrent } };
            }
            return club;
        });
        notify();
    ...
    updateClubFunding(clubId: number, fundingData: Funding) {
        clubs = clubs.map(club => {
            if (club.id === clubId) {
                return { ...club, funding: fundingData };
            }
            return club;
        });
        notify();
