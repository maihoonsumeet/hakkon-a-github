...
const transformClub = async (dbClub: any): Promise<Club> => {
  ...
  return {
    ...
    funding: {
      title: dbClub.funding_title || 'General Fund',
      description: dbClub.funding_description || 'Support our club activities and growth!',
      current: dbClub.funding_current,
      goal: dbClub.funding_goal,
      deadline: dbClub.funding_deadline || new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString().split('T')[0],
    },
  ...
  async updateClub(club: Club): Promise<void> {
    const { error } = await supabase
      ...
      .update({
        ...
        funding_title: club.funding.title,
        funding_description: club.funding.description,
        funding_deadline: club.funding.deadline,
      })
      .eq('id', club.id);
